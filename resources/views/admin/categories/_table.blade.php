...
<tr class="transition-colors duration-300 hover:bg-slate-50">
    ...
    <td class="px-6 py-3 text-gray-500">{{ $category->description ? Str::limit($category->description, 40) : '-' }}</td>
    <td class="px-6 py-3">
    ...
    <td class="px-6 py-3 text-right space-x-2 whitespace-nowrap">
        ...
        <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="inline" onsubmit="return confirm('删除该分类？');">
            @csrf @method('DELETE')
...
@empty
<tr><td colspan="5" class="px-6 py-8 text-center text-gray-400">暂无分类</td></tr>
@endforelse
