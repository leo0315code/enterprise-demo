...
<div class="bg-white rounded-xl shadow-sm overflow-hidden max-w-2xl">
    ...
    <div class="divide-y divide-gray-100">
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="flex items-center gap-3 px-6 py-3">
            @csrf @method('PUT')
            ...
            <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" onsubmit="return confirm('删除该分类？');">
                @csrf @method('DELETE')
    ...
    <!-- 新增 -->
    <form action="{{ route('admin.categories.store') }}" method="POST" class="flex items-center gap-3 px-6 py-4 bg-gray-50 border-t border-gray-100">
        @csrf
