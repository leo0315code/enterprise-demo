...
<tr class="transition-colors duration-300 hover:bg-slate-50">
    <td class="px-6 py-3">
        @if($product->thumbnail)
            <img src="{{ $product->thumbnail }}" alt="{{ $product->title }}" class="w-12 h-12 object-cover rounded" onerror="this.outerHTML='<div class=\'w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded flex items-center justify-center text-sm text-slate-400\'>📦</div>'">
        @else
